...
// Every manifest field holding a path, and therefore needing the app base
// prefixed at import time.
const MANIFEST_PATH_KEYS = ['src', 'poster', 'jpg']
...
function main() {
  ...
  // Entries are built and cached with root-absolute paths ("/media/x.webp"),
  // but they are EMITTED relative ("media/x.webp") and get import.meta.env
  // .BASE_URL prefixed at import time. The site is served from the GitHub
  // Pages project subpath /Max-portfolio/, where a root-absolute path 404s;
  // see `base` in vite.config.js. Keeping the cache absolute means an existing
  // .optimize-cache.json stays valid across this change.
  const relativeManifest = {}
  for (const [key, entry] of Object.entries(sortedManifest)) {
    const out = { ...entry }
    for (const pathKey of MANIFEST_PATH_KEYS) {
      if (out[pathKey]) out[pathKey] = out[pathKey].replace(/^\//, '')
    }
    relativeManifest[key] = out
  }

  const fileContents = `${banner}
//
// Paths below are relative to the app base. BASE_URL is '/' at the domain root
// and '/Max-portfolio/' on GitHub Pages, so one manifest serves both.
const BASE = import.meta.env.BASE_URL

const files = ${JSON.stringify(relativeManifest, null, 2)}

const PATH_KEYS = ${JSON.stringify(MANIFEST_PATH_KEYS)}

const mediaManifest = Object.fromEntries(
  Object.entries(files).map(([key, entry]) => {
    const resolved = { ...entry }
    for (const pathKey of PATH_KEYS) {
      if (resolved[pathKey]) resolved[pathKey] = BASE + resolved[pathKey]
    }
    return [key, resolved]
  }),
)
